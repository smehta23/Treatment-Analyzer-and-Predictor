...
for i, patient in enumerate(patients_set, 0): #feature scaling all columns including the times column 
    ...
    patient[patient_states_parameters] = standardScale.fit_transform(patient[patient_states_parameters])
...
maxEuclideanDistance = sqrt(reducedDimensions) #two columns have been removed 



#---------------------------SELECTING X TEST TIMESTEPS FROM THE TESTING_TIMESTEPS CSV------------------------------------------------------#

num_of_test_timesteps = 3
        ...
        for rowNum, row in enumerate(constantReader, 1):
            ...
            #the varying loop (timestep being compared to constant timestep)
                ...
                if (len(nearest_neighbors_raw_treatments) > 0):
                    ...
                    predictedTreatment_nearest_raw = modeTreatment[0]
                    predictedTreatment_nearest_raw_occurence = modeTreatment[1]
                ...
                if (len(nearest_neighbors_ID_treatments) > 0):
                    ...
                    predictedTreatment_nearest_ID = modeTreatment [0]
                    predictedTreatment_nearest_ID_occurence = modeTreatment [1]
                ...
                if(len(nearest_neighbors_overall_treatments) > 0):
                    ...
                    modeTreatment = list(col.Counter(nearest_neighbors_overall_treatments).most_common(1)[0])
                    predictedTreatment_nearest_overall = modeTreatment[0]
                    predictedTreatment_nearest_overall_occurence = modeTreatment[1]
                ...
                predictedTreatment_K_raw = modeTreatment[0]
                predictedTreatment_K_raw_occurence = modeTreatment[1]
                
                TUPLE_K_neighbors_ID_treatments =  map(tuple, K_neighbors_ID_treatments)
                modeTreatment = list(col.Counter(TUPLE_K_neighbors_ID_treatments).most_common(1)[0])
                predictedTreatment_K_ID = modeTreatment[0]
                predictedTreatment_K_ID_occurence = modeTreatment[1]

                #TUPLE_K_neighbors_overall_treatments = map(tuple, K_neighbors_overall_treatments)
                modeTreatment = list(col.Counter(K_neighbors_overall_treatments).most_common(1)[0])
                predictedTreatment_K_overall = modeTreatment[0]
                predictedTreatment_K_overall_occurence = modeTreatment[1]
